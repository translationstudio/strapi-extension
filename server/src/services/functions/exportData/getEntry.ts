...
const requireTargetEntry = async (contentTypeID: string, entryID: string | undefined, locale: string, logError = true) => {
    try {
...
const getEntry = async (contentTypeID: string, entryID: string | undefined, locale: string, logError = true) => {
    try {
        const contentType = await strapi.contentTypes[contentTypeID];
        const populateConfig = await buildPopulateConfig(contentType);
